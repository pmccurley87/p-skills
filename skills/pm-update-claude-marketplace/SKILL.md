...
All skills live under the single `pm` plugin. Skill directories and `name:` fields use the `pm-` prefix. Skills are referenced as `pm:pm-<skill-name>` (e.g., `pm:pm-tdd`, `pm:pm-planning`).
...
```bash
cp plugins/pm/skills/pm-<skill-name>/SKILL.md skills/pm-<skill-name>/SKILL.md
```
...
1. Create `plugins/pm/skills/pm-<skill-name>/SKILL.md` with proper frontmatter (`name: pm-<skill-name>`)

2. Create `skills/pm-<skill-name>/SKILL.md` (legacy sync copy)
...
1. Remove `plugins/pm/skills/pm-<skill-name>/` directory
2. Remove `skills/pm-<skill-name>/` directory
